...
class Heater:
    def __init__(self, config, sensor, sensor_config=None):
        ...
        sensor_config.getlist("heater_powers", None)
...
class ControlPositionalPID:
    ...
    def check_busy(self, eventtime, smoothed_temp, target_temp):
        temp_diff = target_temp - smoothed_temp
        return (
            abs(temp_diff) > PID_SETTLE_DELTA or abs(self.prev_der) > PID_SETTLE_SLOPE
        )
...
class PrinterHeaters:
    ...
    def setup_heater(self, config, gcode_id=None, heater_config=None):
        ...
        # Create heater
        self.heaters[heater_name] = heater = Heater(config, sensor, heater_config)
