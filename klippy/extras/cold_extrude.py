...
class ColdExtrude:
    ...
    def set_cold_extrude(
        self, gcmd, index_name, cold_extrude_name, min_extrude_temp_name
    ):
        ...
        cold_extrude = gcmd.get_int(cold_extrude_name, None, minval=0, maxval=1)
        ...
        if cold_extrude is None and min_extrude_temp is None:
            gcmd.respond_info(
                "Cold extrudes are %s (min temp %.2f\xb0C)"
                % (
                    "enabled" if heater.cold_extrude else "disabled",
        ...
        if min_extrude_temp is not None:
            ...
            heater.configfile.set(
                heater.name, "min_extrude_temp", heater.min_extrude_temp
            )
            gcmd.respond_info(
