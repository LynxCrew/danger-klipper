...
class Calibrater:
    ...
    def cmd_AXIS_TWIST_COMPENSATION_CALIBRATE(self, gcmd):
        ...
        if axis == 'X':
            ...
            self.interval_dist = x_axis_range / (sample_count - 1)

            for i in range(sample_count):
                x = start_point[0] + i * self.interval_dist
                y = start_point[1]
                self.nozzle_points.append((x, y))
            ...
            self.interval_dist = y_axis_range / (sample_count - 1)

            for i in range(sample_count):
                x = start_point[0]
                y = start_point[1] + i * self.interval_dist
                self.nozzle_points.append((x, y))
    ...
    def _auto_calibration(self, probe_point):
        ...
        # probe the point
        pos = self.probe.run_probe(self.gcmd)
    ...
    def _calibration(self, probe_points, nozzle_points, interval_dist):
        ...
        manual_probe.ManualProbeHelper(
            ...
            self._manual_probe_callback_factory(
                probe_points, nozzle_points, interval_dist))

    def _manual_probe_callback_factory(self, probe_points,
        nozzle_points, interval_dist):
