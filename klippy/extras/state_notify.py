...
class StateNotify:
    ...
    def _klippy_handler(self, state):
        ...
        if state == "ready":
            ...
            if self.menu:
                self.printer.register_event_handler(
                    "menu:begin", lambda e: self._state_handler("menu_begin", e)
                )
                self.printer.register_event_handler(
                    "menu:exit", lambda e: self._state_handler("menu_exit", e)
                )
            ...
            idle_gcode = (
                config_status["config"]["idle_timeout"].get("gcode", "")
                + self.idle_gcode
            )
    ...
    def _check_printer_printing(self):
        ...
        return self.sdcard.is_active() or (
            self.sdcard.progress()
            and self.sdcard.file_path()
