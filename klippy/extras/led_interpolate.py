...
class LedInterpolate:
    ...
    def cmd_LED_INTERPOLATE(self, gcmd):
        ...
        target_colors = [
            round(gcmd.get_float("RED", 0.0, minval=0.0, maxval=1.0) * 255),
            round(gcmd.get_float("GREEN", 0.0, minval=0.0, maxval=1.0) * 255),
            round(gcmd.get_float("BLUE", 0.0, minval=0.0, maxval=1.0) * 255),
            round(gcmd.get_float("WHITE", 0.0, minval=0.0, maxval=1.0) * 255),
        ]
        runtime = gcmd.get_float("DURATION", 0.0)
        ...
        self.current_colors = [
            [round(c * 255, 2) for c in led] for led in self.led_helper.led_state
        ]
        reactor = self.printer.get_reactor()
