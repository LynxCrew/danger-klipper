...
class DualCarriages:
    ...
    def __init__(self, printer, carriages0, carriages1, axes, safe_dist=-1):
        ...
        self.axes = axes
        self.dc = (
            DualCarriage(printer, carriages0, active=True),
        ...
        for axis in self.axes:
            ...
            self.safe_dist[axis] = min(
                abs(c0.position_min - c1.position_min),
    ...
    def toggle_active_dc_carriage(self, axis, index):
        ...
        if target_dc.mode[axis] != PRIMARY:
            newpos = (
                pos[:axis]
                + [target_dc.get_axis_position(axis, pos)]
                + pos[axis + 1 :]
            )
    ...
    def home(self, homing_state, axis):
        ...
        if (self.get_dc_order(axis, 0, 1) > 0) != self.dc[0].get_carriage(
            axis
        ).get_homing_info().positive_dir:
    ...
    def get_status(self, eventtime=None):
        return {
            ("carriage_%d" % (i,)): dc.mode[self.axes[0]]
            if len(self.axes) == 1
            else dc.mode
            for (i, dc) in enumerate(self.dc)
        }
    ...
    def get_kin_range(self, toolhead, mode, axis):
        ...
        if mode == COPY:
            range_min = max(
                range_min, axes_pos[0] - axes_pos[1] + dc1.position_min
            )
            range_max = min(
                range_max, axes_pos[0] - axes_pos[1] + dc1.position_max
            )
        elif mode == MIRROR:
            if self.get_dc_order(axis, 0, 1) > 0:
                range_min = max(range_min, 0.5 * (sum(axes_pos) + safe_dist))
                range_max = min(range_max, sum(axes_pos) - dc1.position_min)
            else:
                range_max = min(range_max, 0.5 * (sum(axes_pos) - safe_dist))
                range_min = max(range_min, sum(axes_pos) - dc1.position_max)
        else:
            # mode == PRIMARY
    ...
    def cmd_SET_DUAL_CARRIAGE(self, gcmd):
        ...
        if (
            len(axis) != 1
            or axis not in "xy"
            or ord(axis) - ord("x") not in self.axes
        ):
    ...
    def save_dual_carriage_state(self):
        ...
        return {
            ...
            "axes_positions": {
                axis: [dc.get_axis_position(axis, pos) for dc in self.dc]
                for axis in self.axes
            },
        }
        ...
        if move:
            ...
            for i, dc in enumerate(self.dc):
                for axis in self.axes:
                    self.toggle_active_dc_carriage(axis, i)
                    homing_speed = min(
                        homing_speed, dc.get_carriage(axis).homing_speed
                    )
            ...
            for axis in self.axes:
                ...
                if dc_mode != INACTIVE:
                    self.dc[1 - primary_ind].activate(
                        axis, dc_mode, cur_pos[primary_ind]
                    )
    ...
    def __init__(self, printer, carriages, active):
        ...
        for s in steppers:
            ...
            for a in self.ENC_AXES:
                ffi_lib.dual_carriage_set_transform(sk, a, 1.0, 0.0)
    ...
    def apply_transform(self, axis):
        ...
        for sk in self.dc_stepper_kinematics:
            ffi_lib.dual_carriage_set_transform(
                sk, self.ENC_AXES[axis], self.scale[axis], self.offset[axis]
            )

    def activate(self, axis, mode, position, old_position=None):
        old_axis_position = self.get_axis_position(
            axis, old_position or position
        )
        self.scale[axis] = -1.0 if mode == MIRROR else 1.0
        self.offset[axis] = (
            old_axis_position - position[axis] * self.scale[axis]
        )
