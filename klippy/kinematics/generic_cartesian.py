...
def find_eigenvectors(m, ek):
    ...
    for i in range(3):
        ...
        if abs(m[i][i]) < 1e-8:
            ...
            vr[i] = 1.0
            nrm_recipr = 1.0 / math.sqrt(mathutil.matrix_magsq(vr))
...
def mat_eigen(mtm):
    ...
    b, e = mtm[1][1:]
    c = mtm[2][2]
    x1 = a**2 + b**2 + c**2 - a * b - a * c - b * c + 3.0 * (d**2 + f**2 + e**2)
    x2 = (
        -(2.0 * a - b - c) * (2.0 * b - a - c) * (2.0 * c - a - b)
        + 9.0
        * (
            (2.0 * c - a - b) * d**2
            + (2.0 * b - a - c) * f**2
    ...
    phi = math.atan2(math.sqrt(4.0 * x1**3 - x2**2), x2)
    sqrt_x1 = math.sqrt(x1)
    # Closed-form expressions for eigenvalues of a symmetric matrix
    l = [
        li / 3.0 if abs(li) > 1e-8 else 0.0
        for li in [
            a + b + c - 2.0 * sqrt_x1 * math.cos(phi / 3.0),
            a + b + c + 2.0 * sqrt_x1 * math.cos((phi - math.pi) / 3.0),
            a + b + c + 2.0 * sqrt_x1 * math.cos((phi + math.pi) / 3.0),
        ]
    ]
    l.sort(reverse=True)
...
def mat_pseudo_inverse(m):
    ...
    s = [[0.0] * 3 for i in range(3)]
    for i in range(3):
        s[i][i] = 1.0 / math.sqrt(l[i]) if l[i] else 0.0
...
class GenericCartesianKinematics:
    def __init__(self, toolhead, config):
        ...
        if self.dc_carriages:
            pcs = [dc.get_primary_carriage() for dc in self.dc_carriages]
            ...
            for axis in axes:
                self.dc_module.get_dc()[0].activate(
                    axis, idex_modes.PRIMARY, zero_pos
                )
    ...
    def _load_kinematics(self, config):
        carriages = {
            a: MainCarriage(config.getsection("carriage " + a), a)
            for a in "xyz"
        }
        dc_carriages = []
    ...
    def _get_kinematics_coeffs(self):
        ...
        offs = {s.get_name(): [0.0] * 3 for s in self.steppers}
        if self.dc_module is None:
            return [matr[s.get_name()] for s in self.steppers], [
                0.0 for s in self.steppers
            ]
        axes = [dc.get_axis() for dc in self.dc_carriages]
    ...
    def get_status(self, eventtime):
        ...
        return {
            ...
            "stepper_kinematics": {
                s.get_name(): A[i] for i, s in enumerate(self.steppers)
            },
            "toolhead_kinematics": {
                s.get_name(): P[i] for i, s in enumerate(self.steppers)
            },
        }
